    ...
    def solve_problem(self, question: str) -> dict:
        ...
        try:
            ...
            for line in lines:
                ...
                elif current_section == "answer" and line and not line.startswith('**'):
                    if line.strip():
                        final_answer += line + " "
    
            # Create validated response using Pydantic
            validated_solution = MathSolution(
                question=question,
                solution_steps=solution_steps if solution_steps else ["Solution provided above"],
                final_answer=final_answer.strip() if final_answer.strip() else "Please refer to the solution above",
